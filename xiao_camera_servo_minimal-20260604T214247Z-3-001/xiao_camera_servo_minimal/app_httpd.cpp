...
// ===============================
// Minimal servo endpoint test
// ===============================
// This tests whether servos can move while the camera server is running.
// Endpoints:
//   /servo?cmd=status
//   /servo?cmd=set&s=1&a=90
//   /servo?cmd=set&s=2&a=120
//   /servo?cmd=home
//   /servo?cmd=forward
//   /servo?cmd=backward
//   /servo?cmd=doubletap
...
static const int SERVO_CALIBRATION = 90;
static const int SERVO_START1 = 25;
static const int SERVO_START2 = 120;
static const int SERVO_STEP_DELAY_MS = 100;
...
static void servo_write_angle(Servo &servo, int angle) {
  angle = constrain(angle, 0, 180);
  servo.write(angle);
}

static void setup_position() {
  servo_write_angle(servo1, SERVO_START1);
  servo_write_angle(servo2, SERVO_START2);
  vTaskDelay(500 / portTICK_PERIOD_MS);
  log_i("Servo home position set");
}

static void init_servos_once() {
  ...
  setup_position();
  log_i("Servos initialized on GPIO %d and GPIO %d", SERVO1_PIN, SERVO2_PIN);
}

static void scroll_forward() {
  log_i("Servo forward");
  servo_write_angle(servo1, SERVO_START1);
  vTaskDelay(SERVO_STEP_DELAY_MS / portTICK_PERIOD_MS);

  servo_write_angle(servo2, SERVO_CALIBRATION);
  vTaskDelay(SERVO_STEP_DELAY_MS / portTICK_PERIOD_MS);

  servo_write_angle(servo1, 120);
  vTaskDelay(SERVO_STEP_DELAY_MS / portTICK_PERIOD_MS);
...
static void scroll_backward() {
  log_i("Servo backward");
  servo_write_angle(servo1, 120);
  vTaskDelay(SERVO_STEP_DELAY_MS / portTICK_PERIOD_MS);
...
static void double_tap() {
  log_i("Servo doubletap");
  servo_write_angle(servo2, SERVO_CALIBRATION);
  vTaskDelay(SERVO_STEP_DELAY_MS / portTICK_PERIOD_MS);

  servo_write_angle(servo2, SERVO_CALIBRATION + 15);
  vTaskDelay(SERVO_STEP_DELAY_MS / portTICK_PERIOD_MS);
...
static esp_err_t servo_handler(httpd_req_t *req) {
  ...
  if (httpd_query_key_value(buf, "cmd", cmd, sizeof(cmd)) != ESP_OK) {
    free(buf);
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "OK servo endpoint ready");
  }

  if (!strcmp(cmd, "status")) {
    free(buf);
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "OK camera servo endpoint ready");
  }
  ...
  if (!strcmp(cmd, "home")) {
    ...
    setup_position();
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "OK home");
  }
  ...
  if (!strcmp(cmd, "forward")) {
    ...
    scroll_forward();
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "OK forward");
  }
  ...
  if (!strcmp(cmd, "backward")) {
    ...
    scroll_backward();
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "OK backward");
  }

  if (!strcmp(cmd, "doubletap")) {
    free(buf);
    double_tap();
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_sendstr(req, "OK doubletap");
  }

  if (!strcmp(cmd, "set")) {
    if (httpd_query_key_value(buf, "s", s_arg, sizeof(s_arg)) != ESP_OK ||
        httpd_query_key_value(buf, "a", a_arg, sizeof(a_arg)) != ESP_OK) {
      free(buf);
      httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "set requires s and a");
      return ESP_FAIL;
    }

    int which = atoi(s_arg);
    int angle = constrain(atoi(a_arg), 0, 180);
    free(buf);

    char response[64];
    if (which == 1) {
      servo_write_angle(servo1, angle);
      snprintf(response, sizeof(response), "OK set servo1 angle %d", angle);
      httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
      return httpd_resp_sendstr(req, response);
    }
    if (which == 2) {
      servo_write_angle(servo2, angle);
      snprintf(response, sizeof(response), "OK set servo2 angle %d", angle);
      httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
      return httpd_resp_sendstr(req, response);
    }

    httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "s must be 1 or 2");
    return ESP_FAIL;
  }

  free(buf);
  httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "unknown servo cmd");
  return ESP_FAIL;
}
